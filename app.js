...
addDetailsBtn.addEventListener("click", function(){
  ...
  if(itemNameInput.value == "" || itemPriceInput.value < 0 || itemPriceInput.value == "" || itemQuantityInput.value < 0 || itemQuantityInput.value == ""){
    return;
  }

  const totalPrice = Number(itemPriceInput.value) * Number(itemQuantityInput.value);
...
function calculateSubTotal(){
  const cost = document.getElementsByClassName("total-price");
  let subTotal = 0;
